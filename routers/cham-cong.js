...
moment.tz.setDefault('Etc/GMT-7');
...
router.post("", async (req, res) => {
    try {
        ...
        console.log(req.session.user.username != "admin");

        if (req.session.user.username != "admin") {
            // Dữ liệu chấm công của tuần trước sẽ được khóa vào 17h00 thứ 2 tuần kế tiếp
            let currentDay = moment();
            const nextMonday = ngayLam.clone().add(1, "week").startOf("week");
            const lockTime = moment(nextMonday).set("hour", 17).set("minute", 0); // Đặt thời gian khóa là 17:00
            console.log("lockTime\t\t" + lockTime.format("DD/MM/YYYY HH:mm"));
            console.log("currentDay\t\t" + currentDay.format("DD/MM/YYYY HH:mm"));
            console.log("ngayLam\t\t" + ngayLam.format("DD/MM/YYYY HH:mm"));
            console.log("ngayLam.isBefore(currentDay)\t" + ngayLam.isBefore(currentDay));
            console.log("currentDay.isAfter(lockTime)\t\t" + currentDay.isAfter(lockTime));
...
router.put("", async (req, res, next) => {
    ...
    try {
        ...
        if (req.session.user.role !== "admin" && req.session.user._id != event.nguoiLam) {
            return res.status(400).json({
...
router.get("/events", async (req, res) => {
    try {
        ...
        const formattedEvents = events.map((event) => ({
            ...
            start: moment.utc(event.ngayLam).format(MOMENT_FORMAT),
            end: moment.utc(event.ngayLam).format(MOMENT_FORMAT),
        }));
